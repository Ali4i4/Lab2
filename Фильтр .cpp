...
int main()
{
    ...
    int c;
    int change;
    cout <<  endl << "Нажмите " << endl << "1 - для перезаписи таблицы заказов" << endl << "2 - для просмотра таблицы заказов" << endl << "3 - для изменения заказа под номером N" << endl << "4 - для фильтрации по номеру стола" << endl << "6 - выход из приложения" << endl;
    cin >> c;
    while (c != 5)
    {
        if (c == 2)
        ...
        if (c == 4)
        {
            cout << endl << "Введите номер стола: ";
            cin >> filter;
            for (int i = 0; i < n; i++)
            {
                if (stoi(cafe[i][1]) == filter)
                {
                    z = i;
                }
            }
